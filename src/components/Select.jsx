...
function Select({ handleChange }) {
  const selectEl = Object.entries(data).map(([key, value]) => (
    <div key={key} class="flex flex-col gap-2">
      ...
      <select
        class="bg-neutral-900 text-teal-50 text-base font-[inherit] font-black p-4 border-2  border-sky-100 rounded-xl cursor-pointer  hover:bg-pink-950 focus:bg-pink-950"
        name={key}
        id={key}
